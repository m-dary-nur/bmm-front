...
<div class="pt-2 md:pt-4">
   ...
   <VirtualList items={dataFilter} rowHeight={45} classes="h-tableview" let:virtualItems let:index>
      <table class="w-full">
         <thead>
            <tr>
         ...
         <tbody>            
            {#if virtualItems.length > 0}
               {#each virtualItems as data, i (data.id)}
                  <tr class="hover:bg-gray-100">
                     <td class="px-4 py-2 border-t border-gray-200 min-w-42 md:min-w-0">{index + i + 1}</td>
                     {#each heads as head (head.key)}
